...
#
# class ATHandler(ATHandle):
#     def __init__(self, at_port):
#         super().__init__(at_port)
#
#     def get_
...
args = [SPDRP_DEVICEDESC, SPDRP_HARDWAREID, SPDRP_COMPATIBLEIDS, SPDRP_UNUSED0, SPDRP_SERVICE, SPDRP_UNUSED1, SPDRP_UNUSED2, SPDRP_CLASS, SPDRP_CLASSGUID, SPDRP_DRIVER, SPDRP_CONFIGFLAGS, SPDRP_MFG, SPDRP_FRIENDLYNAME, SPDRP_LOCATION_INFORMATION, SPDRP_PHYSICAL_DEVICE_OBJECT_NAME, SPDRP_CAPABILITIES, SPDRP_UI_NUMBER, SPDRP_UPPERFILTERS, SPDRP_LOWERFILTERS, SPDRP_BUSTYPEGUID, SPDRP_LEGACYBUSTYPE, SPDRP_BUSNUMBER, SPDRP_ENUMERATOR_NAME, SPDRP_SECURITY, SPDRP_SECURITY_SDS, SPDRP_DEVTYPE, SPDRP_EXCLUSIVE, SPDRP_CHARACTERISTICS, SPDRP_ADDRESS, SPDRP_UI_NUMBER_DESC_FORMAT, SPDRP_DEVICE_POWER_DATA, SPDRP_REMOVAL_POLICY, SPDRP_REMOVAL_POLICY_HW_DEFAULT, SPDRP_REMOVAL_POLICY_OVERRIDE, SPDRP_INSTALL_STATE, SPDRP_LOCATION_PATHS, SPDRP_BASE_CONTAINERID, SPDRP_MAXIMUM_PROPERTY]
name_args = ["SPDRP_DEVICEDESC", "SPDRP_HARDWAREID", "SPDRP_COMPATIBLEIDS", "SPDRP_UNUSED0", "SPDRP_SERVICE", "SPDRP_UNUSED1", "SPDRP_UNUSED2", "SPDRP_CLASS", "SPDRP_CLASSGUID", "SPDRP_DRIVER", "SPDRP_CONFIGFLAGS", "SPDRP_MFG", "SPDRP_FRIENDLYNAME", "SPDRP_LOCATION_INFORMATION", "SPDRP_PHYSICAL_DEVICE_OBJECT_NAME", "SPDRP_CAPABILITIES", "SPDRP_UI_NUMBER", "SPDRP_UPPERFILTERS", "SPDRP_LOWERFILTERS", "SPDRP_BUSTYPEGUID", "SPDRP_LEGACYBUSTYPE", "SPDRP_BUSNUMBER", "SPDRP_ENUMERATOR_NAME", "SPDRP_SECURITY", "SPDRP_SECURITY_SDS", "SPDRP_DEVTYPE", "SPDRP_EXCLUSIVE", "SPDRP_CHARACTERISTICS", "SPDRP_ADDRESS", "SPDRP_UI_NUMBER_DESC_FORMAT", "SPDRP_DEVICE_POWER_DATA", "SPDRP_REMOVAL_POLICY", "SPDRP_REMOVAL_POLICY_HW_DEFAULT", "SPDRP_REMOVAL_POLICY_OVERRIDE", "SPDRP_INSTALL_STATE", "SPDRP_LOCATION_PATHS", "SPDRP_BASE_CONTAINERID", "SPDRP_MAXIMUM_PROPERTY"]
def num_split(text):
    """\
    ...
    def __lt__(self, other):
        if not isinstance(other, ListPortInfo):
            raise TypeError('unorderable types: {}() and {}()'.format(
                type(self).__name__,
                type(other).__name__))
        return num_split(self.device) < num_split(other.device)
...
def iterate_comports():
    ...
    if not SetupDiClassGuidsFromName(
            ...
            ctypes.pointer(ports_guids_size)):
        raise ctypes.WinError()
    ...
    for index in range(len(GUIDs)):
        ...
        while SetupDiEnumDeviceInfo(g_hdi, index, ctypes.byref(devinfo)):
            ...
            info = ListPortInfo(port_name_buffer.value, skip_link_detection=True)

            for name, arg in dict(zip(name_args, args)).items():
                ...
                if SetupDiGetDeviceRegistryProperty(
                        ...
                        None):
                    # pass
                    print('{}---{}'.format(name, szFriendlyName.value.encode()))
